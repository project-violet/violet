...
`/manifest.webmanifest`는 `standalone` 모드로 실행한다. Chrome에서 설치 후 홈 화면 아이콘으로 실행하면 브라우저 주소창 없이 앱 창으로 열린다. 설치 앱에서는 뷰어의 자동 Fullscreen API 요청을 생략하며, 일반 브라우저 탭에서는 기존 모바일 전체화면 설정을 따른다.

설치할 때는 신뢰할 수 있는 HTTPS 주소를 사용한다. `localhost` 예외는 서버와 브라우저가 같은 기기일 때만 적용된다. 다른 휴대폰에서 Y700에 접속한다면 Y700 서버에 HTTPS 인증서와 TLS 종료 프록시 구성이 필요하다. Tailscale 연결 자체가 HTTP 주소를 HTTPS로 바꾸지는 않는다.

HTTP에서 HTTPS로 주소를 바꾸면 브라우저 저장소가 분리되므로 테마, 뷰어 설정, 검색 대상 임시 기록 등은 다시 설정해야 한다. 서버의 북마크/다운로드 DB는 같은 서버를 사용하면 유지된다.

서비스 워커 캐시는 도입하지 않는다. 설치 자체에 오프라인 캐시는 필수가 아니며, 업데이트 시 이전 번들이 남지 않도록 현재 서버의 파일을 그대로 제공한다.
