      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            _subitem(
                '북마크를 서버에 백업하고 싶어요.',
                '설정탭에서 "테마-Drawer 사용" 밑에 있는 구름 업로드 표시를 누르면 북마크 및 열람기록이 서버로 백업됩니다. 백업 후 다른 기기에서 복원하려면 UserAppId를 반드시 기억해야합니다.' +
                    ' User App Id는 구름 업로드 표시 왼쪽 칸을 누르면 볼 수 있습니다.'),
            _divider(),
            _subitem('다운로드가 너무 느립니다.',
                'DPI Bypass 툴(Intra, 유니콘 등)이 실행중이라면 종료하고 재시도해보세요. 그래도 느리고, 느린이유가 앱 문제라고 생각한다면 오류 및 버그 항목으로 문의주세요.'),
            _divider(),
            _subitem(
                '앱 로딩이 너무 느려요.',
                '"아래 검색이 조금 빨랐으면 좋겠어요."를 따라해보세요. 앱 로딩 시간 중 가장 큰 부분이 인덱싱파일 로딩입니다. ' +
                    ' 태그 리빌딩을 통해 인덱싱파일의 크기를 줄이면 앱 로딩 시간을 최대 20배까지 단축시킬 수 있습니다.'),
            _divider(),
            _subitem('검색할 때 하얀박스나 오류메시지가 나와요',
                '데이터베이스가 고장나서 생긴 문제입니다. 데이터베이스 동기화를 통해 데이터베이스를 복구해주세요!'),
            _divider(),
            _subitem('다운로드 시 누락되는 파일들이 있어요',
                '해당 항목을 길게 눌러서 메뉴를 띄운뒤 Recovery를 눌러주세요. 누락된 파일만 다시 다운로드합니다.'),
            _divider(),
            ...
            _divider(),
            _subitem('위에 나오지않은 다른 문제를 겪고있어요',
