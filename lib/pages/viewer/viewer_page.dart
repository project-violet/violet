...
class _ViewerPageState extends State<ViewerPage>
  ...
  void dispose() {
    ...
    if (_pageInfo.useWeb) {
      Future.forEach<String>(_pageInfo.uris,
          (element) => CachedNetworkImageProvider(element).evict());
    }
    SystemChrome.setEnabledSystemUIMode(SystemUiMode.manual, overlays: [
      SystemUiOverlay.top,
          ...
          onPressed: () async {
            ...
            if (mounted) {
              FlutterToast(context).showToast(
                child: ToastWrapper(
                  icon:
                      _vIsBookmarked.value ? Icons.delete_forever : Icons.check,
  ...
  PhotoViewGalleryPageOptions _buildItem(BuildContext context, int index) {
    if (_pageInfo.useWeb) {
      ...
      return PhotoViewGalleryPageOptions.customChild(
        child: FutureBuilder<Tuple2<Map<String, String>, String>>(
...
class _DoublePointListener extends StatefulWidget {
  ...
  const _DoublePointListener({this.child, this.onStateChanged});
      ...
      onTapDown: (TapDownDetails details) {
        ...
        if (_doubleTapCheckTimer != null && _doubleTapCheckTimer.isActive) {
          ...
          _doubleTapCheckTimer =
              Timer(const Duration(milliseconds: 200), _doubleTapTimerElapsed);
        }
      },
      onTapCancel: () {
