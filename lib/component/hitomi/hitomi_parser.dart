...
class HitomiParser {
  ...
  static Future<Map<String, dynamic>> parseGalleryBlock(String html) async {
    ...
    doc.querySelectorAll('tr')
      .forEach((tabRow) {
        final key = tabRow.nodes.firstWhere((node) => node.text?.trim().isNotEmpty ?? false).text?.trim();
        final value = tabRow.nodes.lastWhere((node) => node.text?.trim().isNotEmpty ?? false);
        switch(key){
          // case 'Group':
          //   group;
          //   break;
          ...
          case 'Language':
            language;
            try{
              language = value.nodes.firstWhere((node) => node.text?.trim().isNotEmpty ?? false).attributes['href'] ?? 'N/A';
              if(language == 'N/A') break;
              language = language.replaceAll('/index-', '').replaceAll('.html', '');
            }catch(_){}
            break;
          case 'Series':
            series;
              ...
              ul.nodes
                ...
                .forEach((li) {
                  li.nodes
                    .where((node) => node.text?.trim().isNotEmpty ?? false)
                    .forEach((a) {
                      var tag = a.attributes['href']?.replaceAll('/tag/', '').replaceAll('-all.html', '').replaceAll('%3A', ':').replaceAll('%20', ' ');
                      if(tag == null) return;
                      if(!tag.contains(':')){
                        tag = 'tag:' + tag;
                      }
    ...
    return {'Title': title, 'Artists': artists, 'Language': language, 'Tags': tags, 'Type': type, 'Series': series};
  }
}
